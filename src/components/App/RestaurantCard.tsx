...
function AppRestaurantCard(props: AppRestaurantCardProps) {
	...
	const to = `/restaurant/${restaurant.name}`;
		...
		<Link to={to} className={classNames}>
			<article>
				<div className="w-full h-64 flex-grow-0 relative">
					...
					{!isOpen && (
						<div className="absolute inset-0 bg-black bg-opacity-80 flex items-center justify-center text-white font-bold">
							<AsleepFilled24 className="mr-3" />
							Closed
						</div>
					)}
				</div>
				<header className="px-6 py-4 text-gray-700 dark:text-gray-200 flex-grow">
				...
				<footer className="px-6 py-3 bg-gray-100 dark:bg-gray-600 dark:bg-opacity-40 text-gray-400 dark:text-gray-400 flex-grow-0">
