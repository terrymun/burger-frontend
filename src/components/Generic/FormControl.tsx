...
function GenericFormControlGroup(props: GenericFormControlGroupProps) {
	...
	const inputWrapperElement = useCallback(node => {
		if (!node) return;

		const inputLikeElement = node.querySelector('input, textarea, select');
	...
	return (
		<div className="mb-4 block lg:flex lg:gap-x-3">
			<label htmlFor={id} className="py-2 lg:flex-grow-0 lg:w-48 lg:text-right cursor-pointer">{props.label}</label>
			<div className="lg:flex-grow" ref={inputWrapperElement}>{props.children}</div>
		</div>
	);
}
